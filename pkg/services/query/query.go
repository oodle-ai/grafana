...
type Service interface {
	...
	QueryData(ctx context.Context, user identity.Requester, skipDSCache bool, reqDTO dtos.MetricRequest, applyOptions ...func(*QueryDataOption)) (*backend.QueryDataResponse, error)
}
...
func (s *ServiceImpl) QueryData(ctx context.Context, user identity.Requester, skipDSCache bool, reqDTO dtos.MetricRequest, applyOptions ...func(*QueryDataOption)) (*backend.QueryDataResponse, error) {
	var opt = &QueryDataOption{}
	...
	parsedReq.forwardHeaders = opt.forwardHeaders
	...
	return s.executeConcurrentQueries(ctx, user, skipDSCache, reqDTO, parsedReq.parsedQueries, applyOptions...)
}
...
func (s *ServiceImpl) executeConcurrentQueries(ctx context.Context, user identity.Requester, skipDSCache bool, reqDTO dtos.MetricRequest, queriesbyDs map[string][]parsedQuery, applyOptions ...func(*QueryDataOption)) (*backend.QueryDataResponse, error) {
	...
	for _, queries := range queriesbyDs {
		...
		g.Go(func() error {
			...
			subResp, err := s.QueryData(ctxCopy, user, skipDSCache, subDTO, applyOptions...)
			if err == nil {
